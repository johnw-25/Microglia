...
Fs = 1.04;
lag = 75; %window
threshold = 6.0; % threshold in standard deviations
influence = 0.99;
videos = size(process_data,1);
frames = max(cellfun('size',process_data(:,3),2));
...
%% Break up into PBS/TMEV ( Processes )
...
%% PBS/TMEV/DPI Peak data ( Processes )
% % % TMEV % % %
...
for jj = 1:size(p_PBS_2_5_dpi,1)
    signal = cell2mat(p_PBS_2_5_dpi(jj,3)) + jj;
    plot(signal);
    hold on
...
xlabel('Frames'); ylabel('Video,dpi,roi'); title('Processes: 2-5 DPI PBS');
...
for jj = 1:size(p_PBS_6_14_dpi,1)
    signal = cell2mat(p_PBS_6_14_dpi(jj,3)) + jj;
    plot(signal);
    hold on
...
xlabel('Frames'); ylabel('Video,dpi,roi'); title('Processes: 6-16 DPI PBS');
...
for jj = 1:size(p_TMEV_2_dpi,1)
    signal = cell2mat(p_TMEV_2_dpi(jj,3)) + jj;
    plot(signal);
    hold on
...
xlabel('Frames'); ylabel('Video,dpi,roi'); title('Processes: 2 DPI TMEV');
...
for jj = 1:size(p_TMEV_5_dpi,1)
    signal = cell2mat(p_TMEV_5_dpi(jj,3)) + jj;
    plot(signal);
    hold on
...
xlabel('Frames'); ylabel('Video,dpi,roi'); title('Processes: 5-6 DPI TMEV');
...
for jj = 1:size(p_TMEV_14_dpi,1)
    signal = cell2mat(p_TMEV_14_dpi(jj,3)) + jj;
    plot(signal);
    hold on
...
xlabel('Frames'); ylabel('Video,dpi,roi'); title('Processes: 14-16 DPI TMEV');
