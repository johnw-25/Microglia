function [soma_data, process_data] = map_ca_rev5()
...
[soma_data, soma_table] = soma_matching(fileName, pathName);
[process_data, pro_table] = pro_matching(fileName, pathName);
...
dF_shifted_int = zeros(size(soma_data,1),soma_frames);
before_soma_timeHitRadius1 = zeros(size(soma_data,1),soma_frames);
...
dF_shifted_int = zeros(size(process_data,1),process_frames);
before_process_timeHitRadius1 = zeros(size(process_data,1),process_frames);
...
soma_TMEV_dF = soma_dF_shifted(soma_TMEV_logicals,:);
...
soma_PBS_dF = soma_dF_shifted(soma_PBS_logicals,:);
...
p_TMEV_dF = process_dF_shifted(p_TMEV_logicals,:);
% extract main table data
...
p_PBS_dF = process_dF_shifted(p_PBS_logicals,:);
...
saveas(gcf,fullfile(pathName,'Soma_PBS'));
% % % RROCESS PBS % % %
...
saveas(gcf,fullfile(pathName,'Process_PBS'));
% % % SOMA TMEV % % %
...
saveas(gcf,fullfile(pathName,'Soma_TMEV'));
% % % PROCESS TMEV % % %
...
saveas(gcf,fullfile(pathName, 'Process_TMEV'));
end
