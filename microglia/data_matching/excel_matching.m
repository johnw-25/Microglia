function [final_sorted_data,excel_table] = excel_matching()
...
% convert best_caroi column  to matlab indeces
test_data.Final_calcium_soma = test_data.Final_calcium_soma+1;
...
excel_structure = excel_structure(~isnan([excel_structure.Best_caROI]));
...
for jj = 1:length(suite2p_names)
    ...
    testroi = [excel_structure(findtest).Best_caROI];
...
final_matched_data = cell(size(excel_table,1),7);
...
final_matched_data(:,6) = excel_table.Somma_process;
