function [TRACES, unpairedTrace] = PairedROI(main_suite2p, fileName, pathName)
...
unpairedTable = test_data(~(pairsIndex),:);
unpairedTrace = unpairedTable.TrackID;
pairsTable = test_data(pairsIndex,:);
suite2p_names = folder_data.name(1:end);
notPaired = {'soma','process'};
