function [final_sorted_data,excel_table] = second_best_matching(fileName, pathName)
...
test_data = readtable(fullfile(pathName,fileName),'Sheet',sheet);

% convert x2nd_caROI column  to matlab indeces
test_data.x2nd_caROI = test_data.x2nd_caROI+1;
...
suite2p_names = folder_data.name(3:end);
...
excel_structure = excel_structure(~isnan([excel_structure.x2nd_caROI]));
...
for jj = 1:length(suite2p_names)
    ...
    testroi = [excel_structure(findtest).x2nd_caROI];
...
final_matched_data = cell(size(excel_table,1),6);
...
final_matched_data(:,6) = excel_table.Somma_process2;
final_matched_data(:,7) = num2cell(excel_table.TimeHitRadius1);
...
[~,ix_TMEV] = sort(TMEV_extracted(:,6));
TMEV_extracted(:,:) = TMEV_extracted(ix_TMEV,:);
...
% can sort either by dpi or soma/process
[~,ix_PBS] = sort(PBS_extracted(:,6));
PBS_extracted(:,:) = PBS_extracted(ix_PBS,:);

sorted_data(1:size(PBS_extracted,1),:) = PBS_extracted;
sorted_data(size(PBS_extracted,1)+1:end,:) = TMEV_extracted;
% % delete 30-31 for normal function
empties = any(cellfun('isempty',sorted_data),2);
sorted_data(empties,:) = [];
sorted_PBS = sortcell(PBS_extracted,[6 5]);
sorted_TMEV = sortcell(TMEV_extracted,[6 5]);
final_sorted_data = cell(size(sorted_data,1),size(sorted_data,2));
final_sorted_data(1:size(PBS_extracted,1),:) = sorted_PBS;
final_sorted_data(size(PBS_extracted,1)+1:end,:) = sorted_TMEV;
end
