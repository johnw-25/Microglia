function [soma_data,excel_table] = soma_matching(fileName, pathName, main_suite2p)
...
% convert Final_calcium_soma column  to matlab indeces
...
index = test_data.BESTsomaexclude0_include1;
excel_structure = excel_structure(index == 1);
% Remove all rows with NaN -- we know these won't match
excel_structure = excel_structure(~isnan([excel_structure.Final_calcium_soma]));
...
for jj = 1:length(suite2p_names)
    ...
    testroi = [excel_structure(findtest).Final_calcium_soma];
...
ROIs = excel_table.Final_calcium_soma;
ROIs = num2cell(ROIs);
...
final_matched_data(:,7) = num2cell(excel_table.Mouse);
...
soma_data = sortrows(sorted_data,5);
end
